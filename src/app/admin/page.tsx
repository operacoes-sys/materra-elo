...
// Credentials are validated client-side; OTP is always sent to the fixed admin email server-side
const ADMIN_USER = 'MObossBIG2005#'
const ADMIN_PASS = 'Atlaswill conquerall!17##'
const ADMIN_EMAIL_HINT = 'lucassouto...@gmail.com' // shown in UI (masked)
...
export default function AdminDashboard() {
  ...
  const otpRef7 = useRef<HTMLInputElement>(null)
  const otpInputRefs = [otpRef0, otpRef1, otpRef2, otpRef3, otpRef4, otpRef5, otpRef6, otpRef7]
  ...
  // Step 1 — validate local credentials then request OTP
  const handleAdminLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/auth/admin-otp', { method: 'POST' })
      const data = await res.json()
      if (!data.ok) throw new Error(data.error || 'Falha ao enviar código.')
      setLoginStep('otp')
      setOtpCountdown(60)
    } catch (err: unknown) {
      setAdminLoginError(err instanceof Error ? err.message : 'Erro ao enviar código.')
    } finally {
  ...
  // OTP digit handlers
  const handleOtpDigitChange = (i: number, val: string) => {
    if (!/^\d?$/.test(val)) return
    const next = [...otpDigits]
    next[i] = val
    setOtpDigits(next)
    if (val && i < 7) otpInputRefs[i + 1].current?.focus()
  }
  const handleOtpKeyDown = (i: number, e: React.KeyboardEvent<HTMLInputElement>) => {
    if (e.key === 'Backspace' && !otpDigits[i] && i > 0) otpInputRefs[i - 1].current?.focus()
  }

  // Step 2 — valida contra nosso store server-side (bypassa rate limit do Supabase)
  const handleOtpSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    const token = otpDigits.join('')
    if (token.length < 8) { setOtpError('Digite o código de 8 dígitos completo.'); return }
    setOtpError('')
    setOtpLoading(true)
    try {
      ...
      const data = await res.json()
      if (!data.ok) throw new Error(data.error || 'Código incorreto ou expirado.')
  ...
  const handleResendOtp = async () => {
    ...
    setOtpError('')
    setOtpDigits(['', '', '', '', '', '', '', ''])
    await fetch('/api/auth/admin-otp', { method: 'POST' })
  }
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase
      ...
      .update({
        ...
        plano: selectedUser.plano_ativo 
          ? (['starter', 'growth', 'business', 'intermediário', 'intermediario', 'pro'].includes(String(selectedUser.plano || '').toLowerCase()) 
             ? selectedUser.plano 
             : (selectedUser.tipo_parte === 'Transportadora' ? 'Intermediário' : 'Starter'))
          : 'Gratuito',
  ...
  const handleConfirmReceipt = async (p: any) => {
    try {
      ...
      const url = `https://wa.me/5562999271816?text=${encodeURIComponent(text)}`
  ...
  // Admin gate — must pass credentials + OTP before anything loads
  if (!adminUnlocked) {
    return (
      <div style={{ minHeight: '100vh', background: '#000', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '20px' }}>
        <div style={{ background: '#0f0f0f', border: '1px solid #222', borderRadius: '16px', padding: '48px 40px', width: '100%', maxWidth: '420px', textAlign: 'center' }}>
          ...
          <p style={{ color: '#666', fontSize: '0.85rem', marginBottom: '28px' }}>
            {loginStep === 'credentials'
              ? 'Acesso restrito — identifique-se para continuar.'
              : '📧 Código enviado para lucassouto...@gmail.com — válido por 5 min.'}
          </p>

          {/* Step indicator */}
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', marginBottom: '28px' }}>
            {['Credenciais', 'Código OTP'].map((label, i) => {
              const active = (i === 0 && loginStep === 'credentials') || (i === 1 && loginStep === 'otp')
              const done = i === 0 && loginStep === 'otp'
              return (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                  <div style={{
                    width: '24px', height: '24px', borderRadius: '50%',
                    background: done ? '#00C851' : active ? 'var(--primary-500)' : '#222',
                    border: `1px solid ${done ? '#00C851' : active ? 'var(--primary-500)' : '#444'}`,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: '11px', fontWeight: 800, color: done || active ? '#000' : '#555',
                  }}>{done ? '✓' : i + 1}</div>
                  <span style={{ fontSize: '11px', color: active ? '#fff' : done ? '#00C851' : '#444', fontWeight: active ? 700 : 400 }}>{label}</span>
                  {i < 1 && <div style={{ width: '32px', height: '1px', background: loginStep === 'otp' ? 'var(--primary-500)' : '#333' }} />}
                </div>
              )
            })}
          </div>
          ...
          {/* Error banner */}
          {(adminLoginError || otpError) && (
            <div style={{ background: 'rgba(239,83,80,0.1)', border: '1px solid #ef5350', color: '#ef5350', padding: '10px 14px', borderRadius: '8px', marginBottom: '16px', fontSize: '0.85rem' }}>
              {adminLoginError || otpError}
            </div>
          )}
          ...
          {loginStep === 'credentials' && (
            <form onSubmit={handleAdminLogin} style={{ display: 'flex', flexDirection: 'column', gap: '14px', textAlign: 'left' }}>
              <div>
                <label style={{ color: '#aaa', fontSize: '0.8rem', display: 'block', marginBottom: '6px' }}>Usuário</label>
                <input
                  type="text"
              ...
              <button
                ...
                style={{ marginTop: '8px', background: adminLoginLoading ? '#333' : 'var(--primary-500)', color: '#000', border: 'none', borderRadius: '8px', padding: '12px', fontWeight: 900, fontSize: '1rem', cursor: adminLoginLoading ? 'wait' : 'pointer', letterSpacing: '0.02em' }}
              >
                {adminLoginLoading ? 'Enviando código...' : 'Continuar'}
              </button>
            </form>
          )}
            ...
            <form onSubmit={handleOtpSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              ...
              <div style={{ display: 'flex', gap: '6px', justifyContent: 'center', alignItems: 'center' }}>
                {otpDigits.map((d, i) => (
                  <>
                    {i === 4 && (
                      <span key={`sep-${i}`} style={{ color: '#444', fontSize: '1.2rem', margin: '0 2px' }}>·</span>
                    )}
                    <input
                      key={i}
                      ref={otpInputRefs[i]}
                      ...
                      style={{
                        ...
                        fontSize: '1.35rem',
                        fontWeight: 800,
                        background: d ? 'rgba(255,215,0,0.05)' : 'rgba(255,255,255,0.02)',
                        border: d ? '1px solid var(--primary-500)' : '1px solid rgba(255,255,255,0.1)',
                        borderRadius: '6px',
                        color: 'var(--primary-500)',
                        outline: 'none',
                        boxShadow: d ? '0 0 10px rgba(255,215,0,0.12)' : 'none',
              ...
              <button
                ...
                style={{ background: (otpLoading || otpDigits.join('').length < 8) ? '#222' : 'var(--primary-500)', color: '#000', border: 'none', borderRadius: '8px', padding: '12px', fontWeight: 900, fontSize: '1rem', cursor: (otpLoading || otpDigits.join('').length < 8) ? 'not-allowed' : 'pointer' }}
              >
                {otpLoading ? 'Verificando...' : 'Entrar no Painel'}
              </button>

              <div style={{ fontSize: '12px', color: '#555', textAlign: 'center' }}>
                Não recebeu?
                {otpCountdown > 0
                  ? <span style={{ marginLeft: '6px', fontFamily: 'monospace' }}>Reenviar em {otpCountdown}s</span>
                  : <button type="button" onClick={handleResendOtp} style={{ background: 'none', border: 'none', color: 'var(--primary-500)', cursor: 'pointer', fontSize: '12px', textDecoration: 'underline', marginLeft: '6px' }}>Reenviar código</button>
                }
              </div>

              <button
                type="button"
                onClick={() => { setLoginStep('credentials'); setOtpError(''); setOtpDigits(['','','','','','','','']) }}
              ...
              <tbody style={{ fontSize: '0.9rem' }}>
                {filteredCadastros.map(c => (
                  <tr key={c.id} style={{ borderBottom: '1px solid #222' }}>
                     ...
                     <td>{c.plano_ativo ? (c.plano || 'Sim') : 'Não'}</td>
                     <td>
                        {c.nivel_selo ? (
                          <div style={{
